...
public function index(Request $request, ReclamationRepository $reclamationRepository, PaginatorInterface $paginator): Response
{
    ...
    // Fetch all reclamations (remove the filter for archived reclamations)
    $queryBuilder = $reclamationRepository->createQueryBuilder('r');

    // Optionally, add search functionality here
    if (!empty($searchQuery)) {
        $queryBuilder->andWhere('r.name LIKE :search OR r.lastname LIKE :search OR r.descrirec LIKE :search')
