...
class ProduitController extends AbstractController
{
    ...
    public function outofstock(EntityManagerInterface $entityManager, SMSService $smsService): Response
    {
        // Get the repository
        ...
        $outOfStockProductsString = '';
        foreach ($produits as $produit) {
            if ($produit->getStockP() <10) {
                $outOfStockProductsString .= sprintf(
                    "PRODUCTS OUT OF STOCK\nID: %d, Nom: %s, Date Ajout: %s, Référence: %d\n",
                    $produit->getIdP(),
                    $produit->getNomp(),
                    $produit->getDescrip(),
                    $produit->getPrixp(),
                    $produit->getDateajoutp()->format('Y-m-d'),
                    $produit->getImagep(),
                    $produit->getReferencep()
                );
            }
            $toPhoneNumber = '+21623055345'; 
            $success = $smsService->sendAccountDeactivationSMS($toPhoneNumber,$outOfStockProductsString);
        }

        return new Response($outOfStockProductsString);
    }

    #[Route('/ajouter-au-panier/{idp}', name: 'ajouter_au_panier', methods: ['POST'])]
    public function ajouterAuPanier(int $idp, Request $request, EntityManagerInterface $entityManager, SMSService $smsService): JsonResponse
    {
        ...
        $entityManager->flush();
        $this->outofstock($entityManager, $smsService);
        //$toPhoneNumber = '+21623055345'; 
        //$success = $smsService->sendAccountDeactivationSMS($toPhoneNumber);
       $success =1;
        if (!$success) {
            return new JsonResponse(['success' => true, 'message' => 'Produit ajouté au panier, mais échec de l\'envoi du SMS']);
        }
